...
def get_all_attribute(THWDHWTTC,extracted_features)->np.ndarray:
    ...
    if THWDHWTTC == 1:
        ...
        attribute=get_attribute_single(FORWARD_LANE_TO_CHANGE_THW,extracted_features)
    elif THWDHWTTC == 5:#
        attribute=get_attribute_single(FORWARD_LANE_TO_CHANGE_DHW,extracted_features)            
    elif THWDHWTTC == 6:#
        attribute=get_attribute_single(FORWARD_LANE_TO_CHANGE_TTC,extracted_features)  
    elif THWDHWTTC == 7:#
        attribute=get_attribute_single(BACKWARD_LANE_TO_CHANGE_THW,extracted_features)
    elif THWDHWTTC == 8:#
        attribute=get_attribute_single(BACKWARD_LANE_TO_CHANGE_DHW,extracted_features)            
    else:#
        attribute=get_attribute_single(BACKWARD_LANE_TO_CHANGE_TTC,extracted_features)  
                
    total_len=len(attribute)
    attribute,zeros=count_and_remove_zeros(attribute)
    attribute,negitive=count_and_remove_negitive(attribute)
    attribute,biggerthandefault=count_and_remove_biggerthandefault(attribute)
...
def get_all_different_speed(THWDHWTTC,extracted_features):
    ...
    attribute[TOTAL_MEAN],attribute[TOTAL_VAR] = get_all_attribute(THWDHWTTC,extracted_features)
    ...
    attribute[HIGH_+TOTAL_MEAN],attribute[HIGH_+TOTAL_VAR] = get_different_speed_statistic(1,THWDHWTTC,extracted_features)
    ...
    attribute[MIDDLE_+TOTAL_MEAN],attribute[MIDDLE_+TOTAL_VAR] = get_different_speed_statistic(2,THWDHWTTC,extracted_features)
    ...
    attribute[LOW_+TOTAL_MEAN],attribute[LOW_+TOTAL_VAR] = get_different_speed_statistic(3,THWDHWTTC,extracted_features)
...
def analysis_thwdhwttc(THWDHWTTC,extracted_features)->dict:#THWDHWTTC: thw:1 dhw:2 ttc:3
    attribute={}
...
def analysis_LTCthwdhwttc(LTC_THWDHWTTC,extracted_features)->dict:
    attribute={}
    attribute,attri=get_all_different_speed(LTC_THWDHWTTC,extracted_features)
...
def start_end_total(value):
    ...
    if value[IS_COMPLETE_END] and value[IS_COMPLETE_START]:
        total=(value[LC_ACTION_END_LOCAL_FRAME]-value[LC_ACTION_START_LOCAL_FRAME])/25
...
if __name__ == '__main__':
    ...
    Absolute_Path="/home/demian/code/dataset/NGSIM/smoothed_NGSIM_highDstructure/save_file/"
    ...
    for i in range(0,6):
        ...
        save_file_pkl=Absolute_Path+"{}_vehcile_features_{}.pkl".format(LC_BL,File_Id)
    ...
    save_file='/home/demian/code/python/feature_extration/NGSIM_feature_extraction/gap_normal_action.pkl'
