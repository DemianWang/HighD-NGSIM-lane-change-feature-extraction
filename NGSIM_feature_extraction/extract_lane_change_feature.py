    ...
    if LC_preceding and LC_following:
        for i in range(len(global_frame)):
            ...
            if LC_preceding_instance[FRAME][0]<=current_global_frame and current_global_frame<=LC_preceding_instance[FRAME][len(LC_preceding_instance[FRAME])-1]:
                LC_preceding_S_loc=LC_preceding_instance[BBOX][int(current_global_frame-LC_preceding_instance[FRAME][0]),0]
            if LC_following_instance[FRAME][0]<=current_global_frame and current_global_frame<=LC_following_instance[FRAME][len(LC_following_instance[FRAME])-1]:
                LC_following_S_loc=LC_following_instance[BBOX][int(current_global_frame-LC_following_instance[FRAME][0]),0]
    ...
    for i in range(len(global_frame)):
        ...
        if THWDHWTTC == 1:#THW
            if LC_preceding != 0 and LC_preceding in VEHICLE_IN_EACH_FRAME[global_frame[i]]:
                ...
                rtn[i]=(LC_preceding_S_loc-(current_S_loc))/np.abs(instance[X_VELOCITY][i]+EPSILON)
            ...
            if LC_preceding != 0 and LC_preceding in VEHICLE_IN_EACH_FRAME[global_frame[i]]:
                ...
                current_height=instance[BBOX][0,2]
                current_S_loc=features_of_single_LC[S_LOCATION][i]
                LC_preceding_S_velocity=LC_preceding_instance[X_VELOCITY][int(current_global_frame-LC_preceding_instance[FRAME][0])]
    ...
    for i in range(len(global_frame)):
        ...
        if THWDHWTTC == 1:#THW
            if LC_following != 0 and LC_following in VEHICLE_IN_EACH_FRAME[global_frame[i]]:
                ...
                current_S_loc=features_of_single_LC[S_LOCATION][i]-instance[BBOX][0,2]
                
                rtn[i]=(current_S_loc-(LC_following_S_loc))/np.abs(instance[X_VELOCITY][i]+EPSILON)
            ...
            if LC_following != 0 and LC_following in VEHICLE_IN_EACH_FRAME[global_frame[i]]:
                ...
                LC_following_S_velocity=LC_following_instance[X_VELOCITY][int(current_global_frame-LC_following_instance[FRAME][0])]
...
def get_intention_action_moment_attribute(LC_moment_global,instance,features_of_single_LC,tracks,Direction,
    ...
    if LC_preceding_instance:
        LC_pre_localidx=int(LC_start_global_frame-LC_preceding_instance[FRAME][0])
    ...
    if LC_following_instance:    
        LC_fol_localidx=int(LC_start_global_frame-LC_following_instance[FRAME][0])
    ...
    if is_complete_start:   
        if Direction==1:
            ...
            LC_interaction[INTENTION_THW]=LC_interaction[INTENTION_EGO_PRE_S_DISTANCE]/(current_vehicle_SV+EPSILON) if is_pre_localidx else None
            LC_interaction[INTENTION_LTC_FORWARD_THW]=LC_interaction[INTENTION_EGO_LTCP_S_DISTANCE]/(current_vehicle_SV+EPSILON) if is_LC_pre_localidx else None
            LC_interaction[INTENTION_LTC_BACKWARD_THW]=LC_interaction[INTENTION_EGO_LTCF_S_DISTANCE]/(intention_LC_fol_SV+EPSILON) if is_LC_fol_localidx else None
    ...
    if LC_preceding_instance:
        LC_pre_localidx=int(LC_action_start_global_frame-LC_preceding_instance[FRAME][0])
    is_LC_pre_localidx= (LC_preceding_instance and 0<=LC_pre_localidx and LC_pre_localidx<len(LC_preceding_instance[FRAME]))
    if LC_following_instance:    
        LC_fol_localidx=int(LC_action_start_global_frame-LC_following_instance[FRAME][0])
    is_LC_fol_localidx= (LC_following_instance and 0<=LC_fol_localidx and LC_fol_localidx<len(LC_following_instance[FRAME]))
    if preceding_instance:  
        pre_localidx=int(LC_action_start_global_frame-preceding_instance[FRAME][0])
    is_pre_localidx= (preceding_instance and 0<=pre_localidx and pre_localidx<len(preceding_instance[FRAME]))  
    ...
    if is_complete_start:
        if Direction==1:
            ...
            LC_interaction[ACTION_THW]=LC_interaction[ACTION_EGO_PRE_S_DISTANCE]/(actioncurrent_vehicle_SV+EPSILON) if is_pre_localidx else None
            LC_interaction[ACTION_LTC_FORWARD_THW]=LC_interaction[ACTION_EGO_LTCP_S_DISTANCE]/(actioncurrent_vehicle_SV+EPSILON) if is_LC_pre_localidx else None
            LC_interaction[ACTION_LTC_BACKWARD_THW]=LC_interaction[ACTION_EGO_LTCF_S_DISTANCE]/(action_LC_fol_SV+EPSILON) if is_LC_fol_localidx else None
    ...
    if is_complete_start:
        if Direction==1:
            ...
            LC_interaction[LCMOMENT_THW]=LC_interaction[LCMOMENT_EGO_PRE_S_DISTANCE]/(LCmomentcurrent_vehicle_SV+EPSILON) if is_pre_localidx else None
            LC_interaction[LCMOMENT_LTC_FORWARD_THW]=LC_interaction[LCMOMENT_EGO_LTCP_S_DISTANCE]/(LCmomentcurrent_vehicle_SV+EPSILON) if is_LC_pre_localidx else None
            LC_interaction[LCMOMENT_LTC_BACKWARD_THW]=LC_interaction[LCMOMENT_EGO_LTCF_S_DISTANCE]/(LCmoment_LC_fol_SV+EPSILON) if is_LC_fol_localidx else None
...
def get_THWDHWTTC(tracks,instance,THWDHWTTC):
    ...
    for idx,ID in enumerate(instance[PRECEDING_ID]):
        ...
        if ID == 0 :
            ...
            current_preceding_local=int(current_global_frame-preceding_instance[FRAME][0])
            ...
            if THWDHWTTC == 1:
                rtn[idx]=(current_preceding_S_loc-current_preceding_length-current_S_loc)/(current_S_velocity+EPSILON)
            elif THWDHWTTC == 2:
                rtn[idx]=(current_preceding_S_loc-current_preceding_length-current_S_loc)
            else:
                rtn[idx]=(current_preceding_S_loc-current_preceding_length-current_S_loc)/(current_S_velocity-currtnt_preceding_S_velocity+EPSILON)
...
def extraction_in(tracks,static_info,meta_dict)->dict:
    ...
    for idx, instance in enumerate(tracks):
        ...
        features_of_single_LC[INTENTION_LANE]=np.zeros((total_frame_len,),dtype=np.uint8)+instance[LANE_ID][-1]
