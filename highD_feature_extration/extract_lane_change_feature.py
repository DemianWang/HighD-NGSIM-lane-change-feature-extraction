...
def get_distance_to_lane(s_location, meta, intention_lane, Direction, left_right_maneuver) -> np.ndarray:
    ...
    for idx, int_lane in enumerate(intention_lane):
        if Direction == 1:
            ...
            ego_vehicle_loc[idx] = target_lane_coor[-1 *
                                                    int_lane] if left_right_maneuver[idx] == 0 else target_lane_coor[-1*int_lane-1]
            ego_vehicle_loc[idx] = -1*(s_location[idx, 1] +
                                       s_location[idx, 3]/2-ego_vehicle_loc[idx])
...
def get_LC_start_moment_local(features_of_single_LC, LC_moment_local):
    ...
    middle_frame = int((LC_start_moment_local_right_boundary +
                       LC_start_moment_local_left_boundary)/2)
    acc_01_moment = None
...
def get_LC_action_start_local(features_of_single_LC, LC_start_localidx, is_complete_start):
    ...
    if is_complete_start:
        ...
        for i in range(total_len-LC_start_localidx):
            diff = np.abs(
                features_of_single_LC[D_LOCATION][LC_start_localidx+i]-origin_location)
            if diff >= 0.1:
...
def get_intention_action_moment_attribute(LC_moment_global, instance, features_of_single_LC, tracks, Direction,
    ...
    if is_complete_start:
        if Direction == 1:
            ...
            intention_LC_pre_tail_S = LC_preceding_instance[BBOX][LC_pre_localidx,
                                                                  0] if is_LC_pre_localidx else -1000
            intention_LC_pre_SV = LC_preceding_instance[X_VELOCITY][
                LC_pre_localidx] if is_LC_pre_localidx else 0

            intention_pre_tail_S = preceding_instance[BBOX][pre_localidx,
                                                            0] if is_pre_localidx else -1000
            intention_pre_SV = preceding_instance[X_VELOCITY][pre_localidx] if is_pre_localidx else 0

            intention_LC_fol_head_S = LC_following_instance[BBOX][LC_fol_localidx, 0] + \
                LC_following_instance[BBOX][LC_fol_localidx,
                                            2] if is_LC_fol_localidx else 1400
            intention_LC_fol_SV = LC_following_instance[X_VELOCITY][
                LC_fol_localidx] if is_LC_fol_localidx else 0

            current_vehicle_head_S = instance[BBOX][LC_start_localidx,
                                                    0]+instance[BBOX][LC_start_localidx, 2]
    ...
    if LC_preceding_instance:
        LC_pre_localidx = LC_action_start_global_frame - \
            LC_preceding_instance[FRAME][0]
    is_LC_pre_localidx = (LC_preceding_instance and 0 <=
                          LC_pre_localidx and LC_pre_localidx < len(LC_preceding_instance[FRAME]))
    if LC_following_instance:
        LC_fol_localidx = LC_action_start_global_frame - \
            LC_following_instance[FRAME][0]
    is_LC_fol_localidx = (LC_following_instance and 0 <=
                          LC_fol_localidx and LC_fol_localidx < len(LC_following_instance[FRAME]))
    if preceding_instance:
        pre_localidx = LC_action_start_global_frame - \
    ...
    if is_complete_start:
        if Direction == 1:
            ...
            action_LC_pre_tail_S = LC_preceding_instance[BBOX][LC_pre_localidx,
                                                               0] if is_LC_pre_localidx else -1000
            action_LC_pre_SV = LC_preceding_instance[X_VELOCITY][LC_pre_localidx] if is_LC_pre_localidx else 0

            action_pre_tail_S = preceding_instance[BBOX][pre_localidx,
                                                         0] if is_pre_localidx else -1000
            action_pre_SV = preceding_instance[X_VELOCITY][pre_localidx] if is_pre_localidx else 0

            action_LC_fol_head_S = LC_following_instance[BBOX][LC_fol_localidx, 0] + \
                LC_following_instance[BBOX][LC_fol_localidx,
                                            2] if is_LC_fol_localidx else 1400
            action_LC_fol_SV = LC_following_instance[X_VELOCITY][LC_fol_localidx] if is_LC_fol_localidx else 0

            actioncurrent_vehicle_head_S = instance[BBOX][LC_action_start_localidx,
                                                          0]+instance[BBOX][LC_action_start_localidx, 2]
    ...
    if is_complete_start:
        if Direction == 1:
            ...
            LCmoment_LC_pre_tail_S = LC_preceding_instance[BBOX][LC_pre_localidx,
                                                                 0] if is_LC_pre_localidx else -1000
            LCmoment_LC_pre_SV = LC_preceding_instance[X_VELOCITY][LC_pre_localidx] if is_LC_pre_localidx else 0

            LCmoment_pre_tail_S = preceding_instance[BBOX][pre_localidx,
                                                           0] if is_pre_localidx else -1000
            LCmoment_pre_SV = preceding_instance[X_VELOCITY][pre_localidx] if is_pre_localidx else 0

            LCmoment_LC_fol_head_S = LC_following_instance[BBOX][LC_fol_localidx, 0] + \
                LC_following_instance[BBOX][LC_fol_localidx,
                                            2] if is_LC_fol_localidx else 1400
            LCmoment_LC_fol_SV = LC_following_instance[X_VELOCITY][LC_fol_localidx] if is_LC_fol_localidx else 0

            LCmomentcurrent_vehicle_head_S = instance[BBOX][local_LC_moment,
                                                            0]+instance[BBOX][local_LC_moment, 2]
...
def extraction_in(tracks, static_info, meta_dict) -> dict:
    ...
    for idx, instance in enumerate(tracks):
        ...
        features_of_single_LC[SCENE_AVERAGE_SPEED] = np.float32(
            scene_average_speed(static_info, static, idx+1))  # unit: m/s

        # Local environment information of target (ego) vehicle
        features_of_single_LC[CURRENT_LANE_WIDTH] = get_current_lane_width(
            instance, meta, Direction).astype(np.float32)
        features_of_single_LC[TOTAL_NUM_LANE] = np.uint8(
            get_total_number_lane(Direction))
        features_of_single_LC[ROAD_SPEED_LIMIT] = np.float32(
            meta[SPEED_LIMIT]/3.6) if meta[SPEED_LIMIT] != -1 else np.float32(100)  # unit: m/s if no speed limit, default 100
        features_of_single_LC[CURRENT_ROAD_CURVATURE] = np.zeros(
        ...
        features_of_single_LC[CURRENT_LANE_FEATURE_ID] = get_feature_lane_id(
            instance, Direction)
        features_of_single_LC[CURRENT_LANE_AVERAGE_SPEED] = get_current_lane_average_speed(
            tracks, instance, static_info, static, idx+1)
        features_of_single_LC[CURRENT_LANE_SPEED_LIMIT] = get_current_lane_speed_limit(
            features_of_single_LC[CURRENT_LANE_FEATURE_ID], get_limit_speed(features_of_single_LC[TOTAL_NUM_LANE], meta[SPEED_LIMIT]))
        features_of_single_LC[INTENTION_LANE] = np.zeros(
            (total_frame_len,), dtype=np.uint8)+features_of_single_LC[CURRENT_LANE_FEATURE_ID][total_frame_len-1]
        ...
        features_of_single_LC[D_VELOCITY] = get_vehicle_d_velotity(
            Direction, instance).astype(np.float32)
        features_of_single_LC[HEADING_ANGLE] = np.arctan(features_of_single_LC[D_VELOCITY]/(
            features_of_single_LC[S_VELOCITY]+EPSILON)).astype(np.float32)  # Heading angle along the current lane
        ...
        features_of_single_LC[D_ACCELERATION] = get_vehicle_d_acceleration(
            Direction, instance).astype(np.float32)
        features_of_single_LC[YAW_RATE] = np.arctan(features_of_single_LC[D_ACCELERATION]/(
            features_of_single_LC[S_ACCELERATION]+EPSILON)).astype(np.float32)
        features_of_single_LC[S_JERK] = get_vehicle_jerk(
            features_of_single_LC[S_ACCELERATION], meta).astype(np.float32)
        features_of_single_LC[D_JERK] = get_vehicle_jerk(
            features_of_single_LC[D_ACCELERATION], meta).astype(np.float32)
        ...
        features_of_single_LC[LEFT_LANE_TYPE] = get_lane_type(
            features_of_single_LC[CURRENT_LANE_FEATURE_ID], 0, features_of_single_LC[TOTAL_NUM_LANE])
        features_of_single_LC[RIGHT_LANE_TYPE] = get_lane_type(
            features_of_single_LC[CURRENT_LANE_FEATURE_ID], 1, features_of_single_LC[TOTAL_NUM_LANE])
        features_of_single_LC[STEERING_RATE_ENTROPY] = get_steering_rate_entropy(
            features_of_single_LC[HEADING_ANGLE])
        ...
        LC_end_localidx, is_complete_end = get_LC_end_moment_local(
            features_of_single_LC, LC_moment_local)
        ...
        LC_action_end_localidx, is_complete_action_end = get_LC_action_end_local(
            features_of_single_LC, LC_end_localidx, is_complete_end)
        ...
        LC_action_end_global_frame = int(
            instance[FRAME][LC_action_end_localidx])
